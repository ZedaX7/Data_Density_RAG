...
def run_sanity_check():
    print("\n🧪 Running PyBaMM Sanity Check...")

    features = {
        "series_count": 7,
        "parallel_count": 2,
        "capacity_ah": 5.0,
        "nominal_voltage": 25.9  # 3.7 * 7
    }

    result = validate_with_pybamm(features)
    print("✅ Sanity check result:", "PASS" if result else "FAIL")


class Synthetic_Battery_Pack_Dataset(Dataset):
    def __init__(self):
        self.data = []
        ...
        # Enumerate all series and parallel configurations
        for series in range(1, MAX_SERIES + 1):
            for parallel in range(1, MAX_PARALLEL + 1):
                ...
                # For hexagonal packing, each layer has w*d cells
                for num_cells_w in range(1, MAX_CELLS_WIDTH + 1):
                    for num_cells_d in range(1, MAX_CELLS_DEPTH + 1):
                        ...
                        for num_cells_h in range(min_h, max_h + 1):
                            ...
                            # cell_connections = generate_series_parallel_connections(
                            #     series, parallel, cell_positions
                            # )
                            ...
                            if validate_with_pybamm(features):
                                ...
                                sample = {
                                    ...
                                    "generation_method": "exhaustive_hexagonal_enumeration"
                                }
                                self.data.append(sample)
                                valid_designs += 1

                                if valid_designs % 100 == 0:
                                    elapsed_time = time.time() - start_time
                                    print(f"  Generated {valid_designs} valid designs... (Elapsed: {elapsed_time:.1f}s)")

        end_time = time.time()
        total_time = end_time - start_time
        print(f"\n✅ Generated {len(self.data)} valid designs from {total_attempts} attempts")
        print(f"   Success rate: {100*len(self.data)/max(total_attempts, 1):.1f}%")
...
if __name__ == "__main__":
    dataset = Synthetic_Battery_Pack_Dataset()
    if len(dataset) > 0:
